...
public class PrefixSumBruteForce {
    ...
    public static int prefixSumApproach(int arr[][], int r1, int c1, int r2, int c2) {
        int sum = 0;
        for (int i = r1; i <= r2; i++) {
            for (int j = c1; j <= c2; j++) {
                sum += arr[i][j];
    ...
    public static void main(String[] args) {
        ...
        int[][] arr = new int[m][n];
        System.out.println("enter the array/matrix elements : ");
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                arr[i][j] = sc.nextInt();
            }
        ...
        int result = prefixSumApproach(arr, r1, c1, r2, c2);
        System.out.println("sum of given coordinates is : " + result);
    }
}
